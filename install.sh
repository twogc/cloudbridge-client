...
DOWNLOAD_URL=""
INVITE_TOKEN=""
CONTROL_PLANE_URL="https://edge.2gc.ru"
...
setup_config() {
    ...
    # If invite token provided, use automatic configuration
    if [[ -n "$INVITE_TOKEN" ]]; then
        configure_with_invite_token
        return
    fi
...
configure_with_invite_token() {
    log_info "Configuring client with invite token..."

    # Check if binary exists
    if [[ ! -f "$INSTALL_DIR/cloudbridge-client" ]]; then
        log_error "CloudBridge client binary not found at $INSTALL_DIR/cloudbridge-client"
        return 1
    fi

    # Run join command
    log_info "Running automatic configuration..."
    if sudo "$INSTALL_DIR/cloudbridge-client" join \
        --token "$INVITE_TOKEN" \
        --control-plane "$CONTROL_PLANE_URL" \
        --config "$CONFIG_DIR/config.yaml" \
        --force; then

        log_success "Client configured successfully via invite token!"

        # Set ownership
        sudo chown "$SERVICE_USER:$SERVICE_GROUP" "$CONFIG_DIR/config.yaml"
        sudo chmod 600 "$CONFIG_DIR/config.yaml"

        log_info "Configuration file: $CONFIG_DIR/config.yaml"
    else
        log_error "Failed to configure client with invite token"
        log_warning "You can manually configure later with:"
        log_warning "  sudo $INSTALL_DIR/cloudbridge-client join --token YOUR_TOKEN"
        return 1
    fi
}
...
show_next_steps() {
    ...
    if [[ -n "$INVITE_TOKEN" ]]; then
        echo "✅ Client is fully configured and ready to use!"
        echo
        echo "Next steps:"
        echo "1. Start the service: sudo systemctl start cloudbridge-client"
        echo "2. Check status: sudo systemctl status cloudbridge-client"
        echo "3. View logs: sudo journalctl -u cloudbridge-client -f"
    else
        echo "Next steps:"
        echo "1. Configure the client:"
        echo "   Option A: Use invite token (recommended):"
        echo "     sudo $INSTALL_DIR/cloudbridge-client join --token YOUR_INVITE_TOKEN"
        echo
        echo "   Option B: Use configuration wizard:"
        echo "     sudo $INSTALL_DIR/cloudbridge-client init"
        echo
        echo "   Option C: Edit configuration manually:"
    ...
    echo
    echo "For help:"
    echo "  cloudbridge-client --help"
    echo "  cloudbridge-client join --help"
    echo "  cloudbridge-client init --help"
}
    ...
    while [[ $# -gt 0 ]]; do
        case $1 in
            ...
            --help)
                ...
                echo "  --token TOKEN            Invite token for automatic configuration"
                echo "  --control-plane URL      Control plane URL (default: https://edge.2gc.ru)"
